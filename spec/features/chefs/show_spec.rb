...
RSpec.describe 'chef_path', type: :feature do
  describe 'As a visitor, when I visit a chefs show page' do
    ...
    it 'I see the three most popular ingredients that the chef uses in their dishes' do
      ...
      expect(page).to have_content(@ingredient1.name.to_s)
      expect(page).to have_content(@ingredient2.name.to_s)
      expect(page).to have_content(@ingredient3.name.to_s)
      expect(page).to_not have_content(@ingredient4.name.to_s)
      expect(page).to_not have_content(@ingredient5.name.to_s)
      expect(page).to_not have_content(@ingredient6.name.to_s)
      expect(page).to_not have_content(@ingredient7.name.to_s)
      expect(page).to_not have_content(@ingredient8.name.to_s)
      expect(page).to_not have_content(@ingredient9.name.to_s)
      expect(page).to_not have_content(@ingredient10.name.to_s)
    end
  end
end